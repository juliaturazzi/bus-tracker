...
stops_csv_df = pd.read_csv(STOPS_CSV)
...
def get_all_buses_cached(start_time: str, end_time: str):
    """
    Returns all bus data for the (start_time, end_time) window,
    either from the cache if valid, or fetched from remote if missing/expired.
    """
    ...
    if cache_key in cache_dict:
        ...
        if now - cached_entry["timestamp"] < CACHE_EXPIRY:
            return cached_entry["data"]

    data = fetcher.get_buses_data(start_time, end_time)
    cache_dict[cache_key] = {"data": data, "timestamp": now}
    return data
...
async def get_filtered_bus_line(
    ...
    buses_data = get_all_buses_cached(start_time, end_time)
